"""`.playbook` — where we are, what can happen, and what is true while it does, in ONE file.

DECISIONS are the state space (an assignment of answers, never enumerated; an answer can
`activate` further decisions). EVENTS are what can happen, each carrying its own `when` (on the
table only where it holds) and `hint` (what to do until its document shows). Every event holds ONE
document WRITTEN IN — `content: {kind, doc}`, or `{kind, by, docs}` for a closed set that follows
the answers — so a book is pasted, validated and rendered with nothing beside it, and the walk over
it is session-only: no answer is ever stored.

`version: 2` is the line a book carries; a file without it reads the same way.

The check runs, in order: the YAML; the version; keys the format no longer has (`legacy_problems` —
`topics`, `rules`, `view`, a `content` list, a `file` on an entry, and the rest of what the walk
once wrote or read elsewhere); `by` decisions that are not decisions of the book; what a written
entry cannot be; and every written document through its own kind's engine.
"""
...
def read_version(raw: dict, kind: str, latest: int) -> tuple[int, str | None]:
    ...
    if v < latest:
        return latest, f"`version: {js_str(v)}` — a playbook is one file at version {latest}: every document written in, no `topics`, `rules` or `view`. Read as {latest}"
    if v > latest:
...
def _parse_content(x: Any) -> list[Content]:
    o = _raw_one(x)
    if not _raw_written(o):
        return []
...
def parse(text: str) -> Playbook:
    ...
    for i, e in enumerate(arr(get(raw, "events"))):
        ...
        events.append(Event(key=_str(get(o, "key")) or slug_key(label), label=label,
                            content=_parse_content(get(o, "content"))))
    return Playbook(version, decisions, events)
...
def legacy_problems(text: str) -> list[str]:
    ...
    raw = rec(_yaml.load_or_none(text))
    out: list[str] = []
    off = "every document is written into the book"
    if arr(get(raw, "topics")):
        out.append(f"`topics:` — what is always true is the content of an always-on event")
    if arr(get(raw, "rules")):
        out.append("`rules:` — an event carries its own `when` and `hint`, and its document shows once the answers it follows are taken. Move each rule's `process` onto its event as `hint`, and its `when` onto the event")
    if rec(get(raw, "view")):
        out.append("`view:` — the walk is session-only and writes nothing; every decision opens unanswered. Delete the view")
    if arr(get(raw, "library")):
        out.append(f"`library:` is gone — {off}, under its own event")
    if arr(get(raw, "compare")):
        out.append("`compare:` is gone — there is no A/B view any more")
    if arr(get(raw, "materials")) or arr(get(raw, "scales")):
        out.append(f"`materials:` and `scales:` are gone — nothing damps a branch any more; {off}")
    for i, x in enumerate(arr(get(raw, "events"))):
        o = rec(x)
        where = f"event {_str(get(o, 'key')) or _str(get(o, 'label')) or i + 1}"
        if arr(get(o, "inputs")):
            out.append(f"{where}: `inputs:` is gone — what an event needs or captures belongs in its content")
        if present(get(o, "status")):
            out.append(f"{where}: `status` — the document shows when its answers are taken, and `hint` shows until then. Delete it")
        if present(get(o, "sets")):
            out.append(f"{where}: `sets` — an answer is taken on the rail, never by an event. Delete it")
        if not present(get(o, "content")):
            continue
        if is_list(o["content"]):
            out.append(f"{where}: `content` is one document — a mapping, not a list; several sections belong in one brief")
        c = _raw_one(o["content"])
        name = _str(get(c, "label")) or _str(get(c, "key")) or _str(get(c, "file")) or "the content"
        if _str(get(c, "file")):
            out.append(f"{where}: {name} is a file beside the book — write the document into the book as `kind` + `doc`")
        elif not _raw_written(c) and c:
...
def written_entries(doc: Playbook) -> list[tuple[str, Content]]:
...
def written_problems(doc: Playbook) -> list[str]:
    """What a written entry cannot be."""
    out: list[str] = []
    for where, entry in written_entries(doc):
        name = entry.label or entry.key or "the content"
        form = "`docs`" if entry.docs is not None else "`doc`"
        if not entry.kind:
            out.append(f"{where}: {name} has {form} but no `kind` — say which kind renders it (brief, kanban, md, …)")
...
def check(text: str) -> CheckResult:
    ...
    raw = rec(_yaml.load_or_none(text))
    doc = parse(text)
    version, version_problem = read_version(raw, "playbook", LATEST)
    out = [*([version_problem] if version_problem else []), *legacy_problems(text),
           *variation_problems(doc), *written_problems(doc)]
    # Every document is in this file; each goes through its own kind's engine.
    written = 0
    for where, entry in written_entries(doc):
        if not entry.kind:
            continue
        written += 1
        name = f"{where}: {entry.label or entry.key or 'the content'} ({entry.kind})"
        if entry.kind not in KINDS:
            out.append(f"{name}: not a kind the Studio knows — `--kinds` lists them")
            continue
        for at, d in written_docs(entry):
            _, r = check_written(entry.kind, d)
            out.extend(f"{name}{f' [{at}]' if at else ''}: {p}" for p in r.problems)
    summary = f"v{version} · {len(doc.decisions)} decisions, {len(doc.events)} events" + \
        (f", {written} documents" if written else "")
