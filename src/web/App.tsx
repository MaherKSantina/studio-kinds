 ...
 *
 * Everything happens in the browser. The text is checked and rendered on every
 * keystroke by the same `check` the endpoint runs; nothing is fetched, sent,
 * stored or remembered. Opening a file reads it here, with FileReader.
 */
import { useMemo, useRef, useState, type DragEvent } from "react";
import { fileTemplate } from "../../kinds/fileTemplates";
    ...
    <div className="app" onDragOver={(e) => e.preventDefault()} onDrop={onDrop}>
      ...
      <main className={`split show-${pane}`}>
        ...
        <section className="preview">
          ...
          <div className="doc"><Preview kind={kind} text={text} /></div>
        </section>
      </main>

      <footer className="muted">
        Everything runs in this page. Nothing you paste or open is sent, stored or remembered — reload and it is gone.
        The check is the same <code>check(kind, text)</code> that <code>POST /api/check</code> runs, for tools that cannot open a page.
      </footer>
    </div>
  );
}
