...
| a list curated from several places at several times — the items in the file, each with an id; what came later as stages (rules that set fields, a filter, a sort — each under a circumstance the decisions name); the output shown through views | `.pipeline` | one file: the rows as of any stage and the logic that made them one click apart; the pills are taken for the session only; a `.jsonl`, a `.middleware` and `--collect` read its output, a stage or a view |
| a gallery view of data — each item with its images, facts and link, decided over one at a time | `.collection` made from a `.jsonl` view or a `.pipeline` | a snapshot of the rows and a log of decisions with reasons |
