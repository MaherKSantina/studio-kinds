...
/** The app fetches nothing a document names from off this machine unless STUDIO_REMOTE_CONTENT=on. */
const REMOTE_CONTENT = (process.env.STUDIO_REMOTE_CONTENT ?? "off").toLowerCase() === "on";
...
function createWindow() {
  win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // The page reads this off its argv (preload) and fetches remote images only when told; the session below refuses them otherwise.
      additionalArguments: REMOTE_CONTENT ? ["--studio-remote-content=on"] : [],
    },
  });
  if (!REMOTE_CONTENT) {
    // Without STUDIO_REMOTE_CONTENT=on, nothing a document names leaves this machine. The renderer's own files
